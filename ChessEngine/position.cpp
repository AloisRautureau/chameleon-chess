...
void position::addToStack(movestack &stack, movebits move) const {
    ...
    int moveValue{0};
    char moveFlag = getFlag(move);
    if(moveFlag & CAP) {
        moveValue += Chameleon::Evaluation::see(*this, move, this->m_side); //Use SEE to take a good guess at the move's value
    }
    if(moveFlag & 0b1000) moveValue += 500; //We also value promotions
